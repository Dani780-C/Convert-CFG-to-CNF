    ...
    while(getline(f,linie)){  /// se citesc linii sub forma "neterminal(litera mare) -> "productie1" | ...
        ...
        while(i<linie.length()){
            ...
            if(neterminal == "S"){
                ...
                multime[neterminal].push_back(productie);
            }
            i+=3;
        }
    }
...
void pas1(){       /// stergem neterminalele neutilizabile si inaccesibile
    ...
    for(i = multime.begin(); i != multime.end(); i++){
        ...
        if((neterminal != "S" and neterminal != "S'") and inaccesibil(neterminal) == true) terminale_useless.push_back(neterminal);
        if((neterminal != "S" and neterminal != "S'") and neutilizabil(neterminal) == true) terminale_useless.push_back(neterminal);
    }
    for(int j = 0; j < terminale_useless.size(); j++){
        ///cout<<terminale_useless[j];
...
void pas2(){
    ...
    j++;
    while(j < multime["S"].size() and multime["S"][j] != "#") productii_ramase.push_back(multime["S"][j++]);
    if(productii_ramase.size() > 0){
    if(productii_ramase.size() < multime["S"].size()){
        for(int j = 0; j < productii_ramase.size(); j++){
            string productie = productii_ramase[j];
            int idx = 0;
            string cuv1 = "";
            string cuv2 = "";
            while(idx < productie.size()){
                if(productie[idx] != 'S') cuv1 += productie[idx], cuv2 += productie[idx];
                else{
                    int idx_1 = idx + 1;
                    string cuv3 = "";
                    cuv2 += productie[idx];
                    while(idx_1 < productie.size()) cuv3 += productie[idx_1++];
                    if(verifica_daca_apare("S", cuv1 + cuv3) == true and (cuv1 + cuv3) != "S") productii_ramase.push_back(cuv1 + cuv3);
                    cuv1 = cuv2;
                }
                idx++;
            }
            ...
            if(i->first != j->first){
                if((i->second).size() == (j->second).size())
                    if(egale(i->first,j->first) == true){
                        ...
                        schimba(to_string((i->first)[2]),to_string((j->first)[2]));
                        trebuie_sterse.push_back(j->first);
                    }
            }
        }
        for(int k = 0; k < trebuie_sterse.size(); k++){
            //cout<<trebuie_sterse[k]<<" ";
            multime[trebuie_sterse[k]].clear();
            multime.erase(trebuie_sterse[k]);
        }
        trebuie_sterse.clear();
    }
    for(int k = 0; k < trebuie_sterse.size(); k++){
        //cout<<trebuie_sterse[k]<<" ";
        ///multime[trebuie_sterse[k]].clear();
